...
lm1m <- lm(formula = data$Response ~ data$Enzyme + data$EnzymeConc + 
             data$DetStock + data$Enzyme:data$EnzymeConc + data$Enzyme:data$DetStock + 
             data$EnzymeConc:data$DetStock)

#lm1m <- lm(data$Response~data$Enzyme+data$EnzymeConc+data$DetStock+data$Enzyme:data$EnzymeConc)
Anova(lm1m)

AIC(lm1a, lm1b, lm1c, lm1d, lm1e, lm1f, lm1g, lm1h, lm1i, lm1k, lm1l, lm1m)
BIC(lm1a, lm1b, lm1c, lm1d, lm1e, lm1f, lm1g, lm1h, lm1i, lm1k, lm1l, lm1m)

anova(lm1l, lm1m)
anova(lm1l, lm1i)
# P-value is less than 0.05, the fit is significantly improved for the complex model, hence, we keep the complex model lm1l = lm1m
...
#bc <- boxCox(lm1m, lambda = seq(0, 1, by = 0.05))

# Transforming the response
#data$Response <- ((data$Response^lam)-1)/lam
data$Response <- data$Response^lam #why?
...
step(lm2m, k=3.8)
# We obtain same model as lm2l

AIC(lm2a, lm2b, lm2c, lm2d, lm2e, lm2f, lm2g, lm2h, lm2i, lm2k, lm2l, lm2m)
BIC(lm2a, lm2b, lm2c, lm2d, lm2e, lm2f, lm2g, lm2h, lm2i, lm2k, lm2l, lm2m)
...
lm3m <- lm(data$Response~data$Enzyme+data$EnzymeConc+data$DetStock+data$CaStock+data$Enzyme:data$EnzymeConc+data$EnzymeConc:data$CaStock)
drop1(lm3m, test="F")

lm3m <- update(lm3m, ~.-data$EnzymeConc:data$CaStock)
drop1(lm3m, test="F")

lm3m <- update(lm3m, ~.-data$CaStock)
drop1(lm3m, test="F")
Anova(lm3m)

AIC(lm3a, lm3b, lm3c, lm3d, lm3e, lm3f, lm3g, lm3h, lm3i, lm3k, lm3l, lm3m)
BIC(lm3a, lm3b, lm3c, lm3d, lm3e, lm3f, lm3g, lm3h, lm3i, lm3k, lm3l, lm3m)
...
# data <- data[-159,]
data <- data[-c(147,160),]
lm3l <- lm(data$Response~data$DetStock*data$Enzyme*data$EnzymeConc)
lm3l <- update(lm3l, ~.-data$DetStock:data$Enzyme:data$EnzymeConc)
lm3l <- update(lm3l, ~.-data$DetStock:data$EnzymeConc)
Anova(lm3l)
summary(lm3l)
...
par(mfrow=c(1,1))
plot(data$Response~data$EnzymeConc, col=as.numeric(data$Enzyme), pch=19)
...
# We can't just add runtime 

pairs(data, col=data$RunDate-81100, pch=19)

lm4a <- lm(data$Response~data$RunDate*data$Cycle*data$DetStock*data$CaStock)
Anova(lm4a)
step(lm4a, k=3.8)

lm4a <- lm(data$Response~data$DetStock+data$CaStock+data$DetStock:data$CaStock)
Anova(lm4a)
...
lm4a <- lm(formula = data$Response ~ data$RunDate + data$Cycle + data$DetStock + 
             data$CaStock + data$Cycle:data$DetStock)
drop1(lm4a, test="F")

Anova(lm4a)

lm4a <- update(lm4a, ~.-data$Cycle-data$Cycle:data$DetStock)
Anova(lm4a)

par(mfrow=c(2,2))
plot(lm4a, pch=19, col=as.numeric(data$DetStock)+1)
