...
Plot the different sport disciplines
...
Fit linear model with observation for long.jump 1968. 

```{r}
lmHighJump <- lm(sport$high.jump~sport$year, data=sport)
lmDiscusThrow <- lm(sport$Discus.Throw~sport$year, data=sport)
lmLongJump <- lm(sport$long.jump~sport$year, data=sport)

summary(lmHighJump)
summary(lmDiscusThrow)
summary(lmLongJump)
