...
mix_cols = c("#D8B70A", "#02523B", "#A2A475", "#81A88D", "#000000","#899DA4", "#98E3DD", "#FAEFD1", "#DC863B","#F1BB7B", "#FD6467", "#5B1A18", "#D67236","#DD8D29", "#E2D200", "#46ACC8", "#E58601", "#B40F20","#A42820", "#5F5647", "#9B110E", "#3F5151", "#4E2A1E","#E6A0C4", "#DAF7A6")

# Pairs plot
temp_interval <- cut(df$temp, 4) # divide temperature in intervals to colour
df$month <- as.numeric(as.character(as.Date(df$date, format = "%Y-%m-%d"), format="%m"))
df$month <- factor(df$month)
png(filename="pair_plot_whole.png", width=1750, height=1750, res=300)
...
p1 + scale_color_manual(guide = guide_legend(),values=mix_cols, name="Building type") +  xlab("Temperature ºC") + ylab("Consumption") + theme_classic() + theme(plot.margin = margin(1, 1,0.1, 1, "cm"),)
dev.off()
...
p3 + scale_fill_manual(values=mix_cols) + xlab("Type of building") + ylab("Sum of consumption") + theme_classic()
dev.off()
...
p4 + scale_color_manual(values=mix_cols) + xlab("Date") + ylab("Consumption") + theme_classic() +theme(plot.margin = margin(1, 1,0.1, 1, "cm"),)
dev.off()
